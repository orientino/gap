"""
Training script for GPT-Small on hg38 genomics data.
"""
...
from .model import gdn_small, gpt_small
...
def main():
    ...
    parser.add_argument("--data", type=str, default="hg38_char")
    ...
    k, s = 1, 1
    if args.data.startswith("hg38_kmer"):
        k = int(args.data.split("_")[1][-1])
        s = int(args.data.split("_")[2][-1])
    ...
    if args.m == "gdn":
        model = gdn_small(seq_len=args.seq_len, n_layers=args.n_layers, n_heads=args.n_heads, d_embed=args.d_embed, k=k)  # fmt: skip
    else:
        model = gpt_small(seq_len=args.seq_len, n_layers=args.n_layers, n_heads=args.n_heads, d_embed=args.d_embed, k=k)  # fmt: skip
    model = model.cuda()
    if args.compile:
